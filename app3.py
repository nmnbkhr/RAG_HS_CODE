...
def setup_qa_chain(vectorstore):
    ...
    prompt_template = """Use the following HS code context to answer the question. Provide the exact HS/PCT code, description, and any duty rates or notes. If classifying an item, match it to the closest heading/subheading based on the description. If the item is ambiguous or complex, return a request for clarification instead of guessing.
...
tools = [
    ...
    Tool(
        name="Classify_Item",
        func=classify_item,
        description="Use to classify a single item (e.g., 'Fresh apples') to its HS/PCT code, providing code, reasoning, and notes."
    ),
    Tool(
...
Instructions:
- Always evaluate if the query is clear and complete. If the query is complex, ambiguous, or lacks details (e.g., 'Classify coal' or 'What is the code for apples?'), do not use tools. Instead, use Thought to explain why it's vague, then Final Answer with one clarification question based on context and chat history (e.g., 'Coal can be classified in different ways (e.g., anthracite, bituminous). Please specify the type or form of coal.').
- Do not guess or use tools for vague queries; always ask for clarification first.
- Only if the query is clear (using chat history for context): For a specific HS code query (e.g., 'What is 0101.21?'), use HS_Code_Details and return a descriptive response in Final Answer.
...
- Use chat history to maintain context and build on previous responses (e.g., if user clarified 'fresh apples' in history, use that for follow-ups without asking again).
...
- If no tool is needed (e.g., general question or clarification), respond directly with a clear explanation or question in Final Answer.
- Stop after finding the answer; do not loop unnecessarily. Limit to one response per query.
