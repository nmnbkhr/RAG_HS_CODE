...
def setup_qa_chain(vectorstore):
    ...
    prompt_template = """Use the following HS code context to answer the question. Provide the exact HS/PCT code, description, and any duty rates or notes. If classifying an item, match it to the closest heading/subheading based on the description. If the item is ambiguous or complex, return a list of possible HS codes with descriptions and duties for each, and a request for clarification.
...
def suggest_possible_hs_codes(item_description):
    qa_chain = st.session_state.qa_chain
    query = f"Provide possible HS/PCT codes for '{item_description}', including descriptions and duty rates for each possibility. Use fuzzy matching if necessary to suggest the closest matches."
    result = qa_chain({"query": query})['result']
    return result
...
tools = [
    ...
    Tool(
        ...
        description="Use to get a list of possible HS/PCT codes, descriptions, and duties for a vague or ambiguous item description, using fuzzy matching for suggestions."
    ),
]
...
Instructions:
- Always evaluate if the query is clear and complete using chat history for context. If the query is complex, ambiguous, or lacks details (e.g., 'Classify coal' or 'What is the code for apples?'), use Suggest_Possible_HS_Codes to get suggestions, then in Final Answer, list the possible HS codes with descriptions and duties, and ask for clarification on which one.
- Do not guess for vague queries; use the suggestion tool to provide helpful options, including fuzzy matches if the exact term is unclear.
...
- Stop after one response (either clarification with suggestions or final answer); do not loop unnecessarily.
