...
class WEBOCTariffScraper:
    """Enhanced scraper for WEBOC duty data"""
    ...
    def search_hs_code(self, hs_code: str) -> dict:
        """Search for HS code and extract duty rates"""
    ...
    def parse_duty_table(self, html: str, hs_code: str) -> dict:
        ...
        # Find description
        desc_element = soup.find('span', {'id': re.compile('lblDescription|Description', re.I)})
        if desc_element:
...
def fetch_usd_rate_from_nbp():
    ...
    try:
        ...
        if response.status_code == 200:
            match = re.search(r"US\s*D(?:ollar)?[^0-9]{0,10}([0-9]{2,3}\.?[0-9]{0,4})", response.text, re.IGNORECASE)
            if match:
...
with tab2:
    ...
    with col2:
        ...
        manual_rate = st.number_input(
            ...
            step=0.01,
            value=0.0,
            help="Leave 0 to use automatic rate",
            key="calc_rate"
        )
                ...
                if duty_data.get('status') == 'success':
                    ...
                    breakdown_data = {
                        "Component": [
                            ...
                            f"Additional Duty ({ad_rate}%)" if ad_rate > 0 else None,
                            f"Regulatory Duty ({rd_rate}%)" if rd_rate > 0 else None,
                            f"Sales Tax ({st_rate}%)",
                        ...
                        "Amount": [
                            ...
                            f"{additional_duty:,.2f} PKR" if ad_rate > 0 else None,
                            f"{regulatory_duty:,.2f} PKR" if rd_rate > 0 else None,
                            f"{sales_tax:,.2f} PKR",
                            f"{income_tax:,.2f} PKR",
                            "",
                    ...
                    # Remove None rows
                    breakdown_data["Component"] = [c for c in breakdown_data["Component"] if c is not None]
                    breakdown_data["Amount"] = [a for a in breakdown_data["Amount"] if a is not None]
                    ...
                    st.caption(f"HS Code: {calc_hs_code} - {duty_data.get('description', 'N/A')}")
...
st.markdown("---")
st.caption("💡 Data sources: Pakistan Customs Tariff FY 2024-25 | WEBOC | NBP Exchange Rates")