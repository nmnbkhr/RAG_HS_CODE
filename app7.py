...
def setup_qa_chain(vectorstore):
    llm = OpenAI(temperature=0.4)  # Slightly higher temperature for fuzzy matching

    prompt_template = """Use the following HS code context to answer the question. Provide the exact HS/PCT code, description, and any duty rates or notes (including whether duty is ad valorem (%) or specific (e.g., PKR/kg)). If classifying an item, match it to the closest heading/subheading using fuzzy matching for vague terms. If the item is ambiguous or complex, return a markdown table of possible HS codes with descriptions and duties, and a request for clarification.
...
def suggest_possible_hs_codes(item_description):
    qa_chain = st.session_state.qa_chain
    query = f"Provide a list of possible HS/PCT codes for '{item_description}' using fuzzy matching. Return a markdown table with HS codes, descriptions, and duties for each possibility, followed by a request for clarification."
    result = qa_chain({"query": query})['result']
    return result
...
if "OPENAI_API_KEY" not in os.environ:
    ...
    llm = OpenAI(temperature=0.4)  # Slightly higher for fuzzy matching
    ...
    agent_executor = AgentExecutor(
        ...
        verbose=True,
        handle_parsing_errors="Check the query for ambiguity and provide a clarification table with possible HS codes if needed.",
        max_iterations=10,
