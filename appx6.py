...
class WEBOCTariffScraper:
    ...
    def __init__(self, base_url: str = None):
        ...
        self.hs_code_cache = None  # Cache for HS code list
    ...
    def fetch_hs_code_list(self, search_prefix: str = "") -> list:
        """
        Fetch list of HS codes from WEBOC with autocomplete-style search.
        Returns list of dicts with 'code', 'description', 'unit'
        """
        try:
            ...
            # Look for HS code dropdown, autocomplete list, or table
            hs_codes = []
            
            # Method 1: Find select/dropdown with HS codes
            ...
            # Method 2: Find table with HS codes
            if not hs_codes:
            ...
            return unique_codes[:1000]  # Limit to first 1000
            
        except Exception as e:
            st.warning(f"Could not fetch HS code list: {e}")
            return []
    
    def search_hs_codes_autocomplete(self, query: str, limit: int = 50) -> list:
        """
        Search HS codes with autocomplete functionality.
        Returns matching codes with descriptions and units.
        """
        # If we have cached data, search in it
...
with st.sidebar:
    ...
    if st.session_state.last_search_result:
        ...
        if description and description != 'N/A':
            ...
            st.caption(f"**Item:** {desc_preview}...")
...
with tab2:
    ...
    # Input Section with Interactive HS Code Selector
    st.subheader("📦 Item Details")
    
    # HS Code Input Method Selection
    input_method = st.radio(
        "HS Code Input Method",
        ["🔢 Enter Manually", "📋 Search from WEBOC List"],
        horizontal=True,
        key="hs_input_method"
    )
    ...
    if input_method == "🔢 Enter Manually":
        ...
        with col2:
            if calc_hs_code and st.button("🔍 Fetch Details", use_container_width=True):
    ...
    else:  # Search from WEBOC List
        st.markdown("**Search HS Codes from WEBOC Database**")
        
        search_col1, search_col2 = st.columns([3, 1])
        with search_col1:
            search_query = st.text_input(
                "Search by HS Code or Description",
                placeholder="e.g., 0808 or 'apples' or 'cotton'",
                key="hs_search_query"
            )
        with search_col2:
            st.write("")
        ...
        if search_btn and search_query:
            with st.spinner("Searching WEBOC database..."):
        ...
        if st.session_state.hs_code_options:
            st.caption(f"Found {len(st.session_state.hs_code_options)} matching HS codes")
            
            # Create selectbox with formatted options
            options_display = [
                f"{item['code']} - {item['description'][:80]}... ({item['unit']})"
            ...
            if selected_option is not None:
                ...
                # Fetch full details from WEBOC
                if st.button("✅ Use This HS Code", use_container_width=True, type="primary"):
                    with st.spinner("Fetching complete details..."):
                        weboc_data = st.session_state.weboc_scraper.search_hs_code(calc_hs_code)
                        if weboc_data.get('status') == 'success':
                            st.session_state.selected_hs_info = weboc_data
                            st.success(f"✅ Selected: {calc_hs_code}")
                            st.rerun()
        else:
            st.info("💡 Enter a search term to find HS codes from WEBOC database")
    
    # Show selected HS info if available
    if st.session_state.selected_hs_info:
        ...
        if info.get('status') == 'success':
            ...
            if auto_detected_description and auto_detected_description != 'N/A':
                st.info(f"📝 **Description:** {auto_detected_description}")
            
            duty_info_cols = st.columns(3)
            with duty_info_cols[0]:
                st.metric("Customs Duty", f"{info.get('customs_duty', 0)}%")
            with duty_info_cols[1]:
                st.metric("Sales Tax", f"{info.get('sales_tax', 0)}%")
            with duty_info_cols[2]:
                st.metric("Income Tax", f"{info.get('income_tax', 0)}%")
    
    st.markdown("---")
    
    # Quantity and Value inputs
