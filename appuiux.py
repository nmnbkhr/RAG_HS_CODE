...
def fetch_usd_rate_from_nbp():
    """Scrape USD->PKR rates from NBP - Returns both TT Buying and TT Selling"""
    try:
        response = requests.get(NBP_USD_RATE_URL, timeout=10)
        if response.status_code == 200:
            text = response.text
            
            # Extract both TT Buying and TT Selling rates
            rates = {'tt_buying': None, 'tt_selling': None}
            
            # Patterns for TT Selling (for imports - buying USD)
            selling_patterns = [
            ...
            # Patterns for TT Buying (for exports - selling USD)
            buying_patterns = [
            ...
            # If we found at least one rate, return
            if rates['tt_selling'] or rates['tt_buying']:
                # If one is missing, use the other (typically differ by ~1-2 PKR)
                if not rates['tt_selling'] and rates['tt_buying']:
                    rates['tt_selling'] = rates['tt_buying'] + 2.0  # Approximate spread
        ...
        try:
            ...
            if sbp_response.status_code == 200:
                ...
                if match:
                    rate = float(match.group(1))
                    if 200 < rate < 400:
                        # Use mid-rate with typical spread
                        return {
                            'tt_selling': rate + 1.0,
                            'tt_buying': rate - 1.0
                        }
    ...
    # Hardcoded fallback with typical spread
    return {
        'tt_selling': 280.50,  # For imports (buying USD)
        'tt_buying': 278.50    # For exports (selling USD)
    }

def get_exchange_rate(from_currency, transaction_type='import'):
    """
    Get PKR conversion rate based on transaction type
    transaction_type: 'import' (uses TT Selling) or 'export' (uses TT Buying)
    """
    ...
    if currency == "USD":
        ...
        if transaction_type == 'export':
            # Exports: Bank buys your USD (use TT Buying rate)
            if rates['tt_buying'] and rates['tt_buying'] > 200:
                return rates['tt_buying'], f"NBP TT Buying (Export Rate)"
            return 278.50, "Estimated TT Buying (Export)"
        else:
            # Imports: You buy USD (use TT Selling rate)
            if rates['tt_selling'] and rates['tt_selling'] > 200:
                return rates['tt_selling'], f"NBP TT Selling (Import Rate)"
            return 280.50, "Estimated TT Selling (Import)"
    
    # For other currencies, use market rate
    try:
...
# Tabs
tab1, tab2, tab3 = st.tabs(["🔍 HS Code Lookup", "📥 Import Calculator", "📤 Export Calculator"])
        ...
        if not calc_hs_code or calc_quantity <= 0 or calc_value <= 0:
            ...
            with st.spinner("📊 Calculating duties and taxes..."):
                ...
                if duty_data.get('status') == 'success':
                    # Get exchange rate (TT Selling for imports)
                    if manual_rate > 0:
...
with tab3:
    ...
    **Pakistan Export Rules:**
    ...
    - ⚠️ **Exception:** Regulatory duty may apply (check notifications)
    ...
    st.markdown('<div class="section-header">1️⃣ Export Item Details</div>', unsafe_allow_html=True)
    
    exp_col1, exp_col2 = st.columns(2)
    
    with exp_col1:
        export_hs_code = st.text_input(
            "HS Code",
            placeholder="e.g., 0808.1000",
            key="export_hs_code",
            help="Enter HS/PCT code for export item"
        )
        
        export_description = st.text_input(
            "Item Description",
    ...
    with exp_col2:
        export_unit = st.selectbox(
            "Unit of Measurement",
            ["kg", "units", "liters", "tonnes", "meters", "pairs", "dozens", "sets"],
            key="export_unit"
        )
        
        export_destination = st.selectbox(
            "Destination Country",
            ["UAE", "USA", "UK", "China", "Saudi Arabia", "Afghanistan", "Bangladesh", "Other"],
            key="export_dest",
            help="Export destination for GD"
        )
        
        export_scheme = st.selectbox(
            "Export Scheme",
            ["Normal Export", "DTRE (Duty & Tax Remission)", "Manufacturing Bond", "Export Oriented Unit", "Other"],
            key="export_scheme",
            help="Select applicable export scheme for drawback eligibility"
        )
    
    st.markdown('<div class="section-header">2️⃣ FOB Value & Exchange</div>', unsafe_allow_html=True)
    ...
    with fob_col1:
        export_currency = st.selectbox(
            "Currency",
            ["USD", "EUR", "GBP", "AED", "PKR", "CNY"],
            key="export_curr"
        )
    
    with fob_col2:
        export_fob_per_unit = st.number_input(
            f"FOB Value per {export_unit}",
            min_value=0.0,
            step=0.01,
            key="export_fob_unit",
            help="Free on Board value per unit"
        )
    
    with fob_col3:
        export_manual_rate = st.number_input(
            "Custom TT Buying Rate (optional)",
            min_value=0.0,
            step=0.01,
            value=0.0,
            key="export_manual_rate",
            help="Leave 0 for auto NBP TT Buying rate"
        )
    
    st.markdown('<div class="section-header">3️⃣ Regulatory Duty Check</div>', unsafe_allow_html=True)
    
    rd_col1, rd_col2 = st.columns(2)
    
    with rd_col1:
        has_regulatory_duty = st.checkbox(
            "Regulatory Duty Applicable?",
            value=False,
            key="export_has_rd",
            help="Check if regulatory duty applies per FBR notification"
        )
    
    with rd_col2:
        if has_regulatory_duty:
            regulatory_duty_rate = st.number_input(
    ...
    if st.button("💰 Calculate Export Proceeds", type="primary", use_container_width=True, key="calc_export"):
        if not export_hs_code or export_quantity <= 0 or export_fob_per_unit <= 0:
            st.error("⚠️ Please provide valid HS code, quantity, and FOB value")
        else:
            with st.spinner("📊 Calculating export proceeds..."):
                # Get TT Buying rate for exports
                ...
                regulatory_duty_amount = 0
                if has_regulatory_duty and regulatory_duty_rate > 0:
                    regulatory_duty_amount = total_fob_pkr * (regulatory_duty_rate / 100)
                
                # Net proceeds after regulatory duty
                net_proceeds_pkr = total_fob_pkr - regulatory_duty_amount
                
                # Duty drawback estimation (simplified - actual depends on scheme and SRO)
                drawback_eligible = export_scheme in ["DTRE (Duty & Tax Remission)", "Manufacturing Bond", "Export Oriented Unit"]
                estimated_drawback = 0
                if drawback_eligible:
                    # Typical drawback ranges from 1-7% of FOB value
                    estimated_drawback = total_fob_pkr * 0.03  # 3% estimate
                ...
                with export_result_cols[2]:
                    st.metric("Regulatory Duty", f"PKR {regulatory_duty_amount:,.0f}" if has_regulatory_duty else "PKR 0")
                ...
                # Drawback/Refund Information
                st.markdown("---")
                st.markdown('<div class="section-header">🎁 Duty Drawback / Refund</div>', unsafe_allow_html=True)
                ...
                if drawback_eligible:
                    ...
                    **✅ Eligible for Duty Drawback**
                    ...
                    - **Estimated Drawback:** PKR {estimated_drawback:,.2f} (approx. 3% of FOB)
                    - **Note:** Actual drawback depends on:
                      - Specific SRO notifications
                      - Input duties paid on raw materials
                      - Compliance with scheme conditions
                      - Export documentation completeness
                    
                    💡 **Action Required:** File duty drawback claim in export GD and maintain proper documentation.
                    """)
                else:
                    st.info(f"""
                    **ℹ️ Drawback Status**
                    ...
                    - **Export Scheme:** {export_scheme}
                    - **General Drawback:** May be available if input duties were paid
                    - **Recommendation:** Check eligibility under relevant SRO notifications
                    
                    💡 Consider registering under DTRE or Manufacturing Bond scheme for future exports.
                    """)
                
                # Export GD Summary
                st.markdown("---")
                st.markdown('<div class="section-header">📋 Export GD Summary</div>', unsafe_allow_html=True)
                
                gd_summary_cols = st.columns(2)
                with gd_summary_cols[0]:
                    st.markdown(f"""
                    **Export Declaration Details:**
                    - HS Code: `{export_hs_code}`
                    - Description: {export_description or "As per invoice"}
                    - Quantity: {export_quantity:,.2f} {export_unit}
                    - FOB Value: {total_fob_foreign:,.2f} {export_currency}
                    - Destination: {export_destination}
                    """)
                
                with gd_summary_cols[1]:
                    st.markdown(f"""
                    **Financial Summary:**
                    - Exchange Rate: {tt_buying_rate:.4f} ({rate_source})
                    - PKR Proceeds: PKR {total_fob_pkr:,.2f}
                    - Regulatory Duty: PKR {regulatory_duty_amount:,.2f}
                    - Net Proceeds: PKR {net_proceeds_pkr:,.2f}
                ...
                **⚠️ Important Compliance Notes:**
                1. **Export Duty:** Pakistan generally has NO export duty (confirmed by law)
                2. **Regulatory Duty:** Only if specifically notified by FBR for your HS code
                3. **Proceeds Repatriation:** Must be repatriated within prescribed timeframe
                4. **Documentation:** Maintain export invoice, packing list, bill of lading, and certificate of origin
                5. **Sales Tax:** Zero-rated for exports - maintain proper records for refund claims
                6. **Drawback Claims:** File within prescribed time limits with supporting documents
                """)
                
                st.caption(f"""
                📍 **Calculation Basis:** FOB Value • TT Buying Rate (Exports) • HS Code: {export_hs_code}
                📝 **Description:** {export_description or 'N/A'} • **Destination:** {export_destination}
                """)
...
<div style='text-align: center; color: #6b7280; padding: 2rem 0;'>
    <p><strong>Pakistan Customs - HS Code & Import/Export Calculator</strong></p>
    <p>📥 <strong>Import:</strong> CIF Basis + TT Selling Rate (Buying USD) • 📤 <strong>Export:</strong> FOB Basis + TT Buying Rate (Selling USD)</p>
    <p>Data Sources: Pakistan Customs Tariff FY 2024-25 | WEBOC | NBP Exchange Rates (TT Buying & Selling)</p>
    <p style='font-size: 0.875rem;'>⚠️ For official use, please verify with FBR and WEBOC • Export duty: Generally nil per Pakistan law</p>
</div>
""", unsafe_allow_html=True)